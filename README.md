...
Also it will listen only on local IP addresses : when it determines the IP address to listen on, it aborts if obtained IP address is not a Class C IP address (aka Local Area Network IP address, first member between 192 and 223 inclusive).
